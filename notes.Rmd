---
title: "notes"
author: "Frank Jung"
date: "11 August 2015"
output: 
  pdf_document: 
    fig_crop: no
    latex_engine: xelatex
---

```{r setoptions, echo=TRUE, message=FALSE, warning=FALSE}
# set our runtime environment and global defaults.
require(knitr, quietly = TRUE)
require(utils, quietly = TRUE)
require(ggplot2, quietly = TRUE)
require(scales, quietly = TRUE)
opts_chunk$set(echo = FALSE, cache = TRUE, cache.path = "cache/", fig.path = "figure/")
```

Cumulative means of 1000 exponential distributions with $\lambda$ of 0.2.

```{r cummeans, echo=TRUE, message=FALSE, warning=FALSE}
set.seed(040)
n <- 1000
lambda <- 0.2
means <- cumsum(rexp(n, lambda))/(1:n)
plot(1:n, means, type = "l", lwd = 2, frame = FALSE, ylab = "Cumulative Means", xlab = "Sample Size")
abline(h = 1/lambda)
```


## Simulation of mean of $n$ dice

```{r, echo = FALSE}
par(mfrow = c(1, 3))
for (n in c(1, 2, 6)){
  temp <- matrix(sample(1 : 6, n * 10000, replace = TRUE), ncol = n)
  temp <- apply(temp, 1, mean)
  temp <- (temp - 3.5) / (1.71 / sqrt(n)) 
  dty <- density(temp)
  plot(dty$x, dty$y, xlab = "", ylab = "density", type = "n", xlim = c(-3, 3), ylim = c(0, .5))
  title(paste("sample mean of", n, "obs"))
  lines(seq(-3, 3, length = 100), dnorm(seq(-3, 3, length = 100)), col = grey(.8), lwd = 3)
  lines(dty$x, dty$y, lwd = 2)
}
```

## plot histogram with normal density

A further complication: comparison of the estimated density with the normal
density

The following tricky approach allows to superimpose on each panel two density
plot, one for the observed density and one for the normal density. The approach
exploit a further “apply” function, named plyr (for details look at offcial ply
documentation).
```{r, echo = FALSE}
set.seed(040)
lambda <- 0.2
observ <- 40
samples <- 1000
# simulated data 1000 sample rows by 40 observation columns
simdata <- matrix(sapply(1:samples, function(x) rexp(observ, lambda)), samples, observ)
# means and variances of each sample
simmeans <- apply(simdata, 1, function (x) mean(x[]))
simvars <- apply(simdata, 1, function (x) var(x[]))
mu = 1/lambda
se = sqrt(1/lambda^2)/sqrt(observ)
simnormals <- apply(simdata, 1, function (x) (mean(x[]) - mu)/se)
# put into data frame use with plots
simdf <- data.frame(means = simmeans, vars = simvars, normals = simnormals)

hist(simdf$normals)

# plot mean
ggplot(data = dfSampleMeanUniform, aes(sampleMean)) + geom_histogram(aes(y = ..density..), 
    col = "grey50", fill = "grey") + geom_density() + geom_line(aes(y = dens), 
    data = density, colour = "red", linetype = "dotted", size = 2) + facet_wrap(~size, 
    scales = "free")
```
Source: http://stat.unicas.it/vistoccoDownload/busStat/rCodes/centralLimitTheorem.html

## Appendices

### Session Information

This document was produced in RStudio. The session information detailing
packages used is:

```{r sessioninformation} 
sessionInfo()
```


### Exponential Distribtion

```{r expdis}
samples <- rexp(100, rate = 0.2)
par(fig = c(0, 1, 0, 0.35))

png(filename = "~/documents/wiki/media/frank/datascience/exponential-boxplot.png", height = 200, width = 480)
boxplot(samples, horizontal = T, bty = "n", xlab = "exponential distribution")
dev.off()

par(fig=c(0, 1, 0.25, 1), new = T)
s <- seq(0, max(samples), 0.1)
d <- dexp(s, rate = 0.2)

png(filename = "~/documents/wiki/media/frank/datascience/exponential.png")
hist(samples, prob = T, main = "", col = adjustcolor("purple", alpha = 0.5), ylim = c(0, max(d)))
lines(density(samples), lty = 2)
curve(dexp(x, rate = 0.2), lwd = 2, add = T)
rug(samples)
dev.off()
```

### Poisson Distribution

```{r rpois}
require(ggplot2)

k <- seq(0, 5)
poisson <- data.frame(x = k, y = dpois(k, lambda = 0.5))

png(filename = "~/documents/wiki/media/frank/datascience/poisson.png")
ggplot(poisson, aes(x = x, y = y)) +
    geom_bar(stat = "identity", colour = "purple", alpha = 0.5, fill = "purple") +
    scale_x_discrete(breaks = k) +
    labs(x = "k", y = "P(X=k)") +
    ggtitle("Poisson Distribution, λ = 0.5")
dev.off()
```