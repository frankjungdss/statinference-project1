...
* normalise the results for [later comparision](#distribution-is-approximately-normal) to the standard normal

# Sample Mean versus Theoretical Mean

Using ```r observ``` observations of the exponential distribution repeating this
```r samples``` times, taking the mean for each sample of observations. The mean
of these samples should approach the theoretical mean, ($1/\lambda$), as shown
in the plot below:
...
repeating this ```r samples``` times, taking the variance for each sample of 
...
($n$) increases. Plotting a kernel probability density histogram of these
averages we should see a normal like distribution:
...
To help evaluate how close this is to the own simulation, we have overlayed the 
normalised sample averages with the standard normal distribution (in red),
$N(0,1)$.
